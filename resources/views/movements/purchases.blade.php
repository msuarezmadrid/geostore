...
@section('content')
    ...
    <section class="content-header">
      <h1>
        <i class="fa fa-arrow-left" style="padding-right: 5px;"></i> Ordenes de entrada

         <button type="button" class="btn btn-success btn-xs" id="myButton"> <i class="fa fa-plus" style="padding-right: 5px;"></i> Agregar Orden de Entrada</button> 
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Ordenes de Entrada</li>
      </ol>
    </section>
        ...
        <div class="row">
            <div class="col-xs-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="location" id="supplier_id"></select>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="box box-primary flat box-solid">
                    ...
                    <div class="box-header">
                        <h3 class="box-title"><i class="fa fa-list" style="padding-right: 5px;"></i> Listado de Ordenes de Entrada</h3>  
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <div class="col-md-12">
                          ...
                          <div class="nav-tabs-custom">
                            <ul class="nav nav-tabs">
                              <li  class="active"><a href="#tab_2" data-toggle="tab"><i class="fa fa-exclamation" style="padding-right: 5px;"></i> Ordenes de Entrada Pendientes</a></li>
                              <li><a href="#tab_3" data-toggle="tab"><i class="fa fa-check" style="padding-right: 5px;"></i>Ordenes de Entrada Aprobadas</a></li>
                            </ul>
...
@section('js')
    ...
    <script>
        ...
        var rows = [
            ...
            [
                {field: 'Proveedor:', type: 'combo', id: 's_supplier_id', value: 'Proveedor'},
                {field: 'Almacén', type: 'combo', id: 's_location_id', value: 'Almacén'}
            ]
        ];

        var params = {
            title: 'Crear Orden de Entrada',
            rows: rows,
            button: 'Agregar'
        }
        ...
        $(document).ready(function() {
            ...
            table = $('#datas').DataTable( {
                ...
                "ajax": function (data, callback, settings) {
                    ...
                    filters.supplier_id = $('#supplier_id').val();
                    data.filters = filters;

                    //extra data

                    AWApi.get('{{ url('/api/purchases' ) }}?' + $.param(data), function (response) {
                        callback({
                ...
                "columns": [
                    ...
                    { "data": "id",
                        className:'text-center',
                        render: function ( data, type, full, meta ) {
                            return full.supplier.razon_social;
                        }
                    },
            ...
            (function setFieldValues() {
                ...
                fieldValues['supplier_id'] = 's_supplier_id';
            ...
            $('#myButton').click(function () {
                ...
                AWApi.get('{{ url('/api/suppliers') }}',function (response) {
                    for (var i = 0; i < response.data.suppliers.length; i++) {
                        $('<option />', {value: response.data.suppliers[i].id, text: response.data.suppliers[i].razon_social }).appendTo($("#s_supplier_id")); 
                    }
                    $("#s_supplier_id").append($("#s_supplier_id option").remove().sort(function(a, b) {
                        var at = $(a).text(), bt = $(b).text();
                        return (at > bt)?1:((at < bt)?-1:0);
                    }));

                    $('#s_supplier_id').selectpicker('refresh');
            ...
            $('#' + modal_add_purchase + "_create").click(function(){
                ...
                data.append('supplier_id', $('#s_supplier_id').val());
                data.append('movement_status_id', 2);

                AWApi.post('{{ url('/api/purchases') }}', data, function(datas){
                    if(datas && datas.data.errors) {
                        let errList = generateErrorList(datas.data.errors);
                        swal("Error", errList, "error");
                    } else if(datas && datas.success && datas.success == true && datas.data) {
                        submit(modal_add_purchase, datas, "Agregar Orden de Entrada");
                    }
