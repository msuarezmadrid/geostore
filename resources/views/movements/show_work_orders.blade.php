...
@section('content')
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{url('work_orders')}}">Ordenes de Trabajo</a></li>
        <li class="active" id="breadcrumb_active">Orden de Trabajo</li>
      </ol>
    </section>
              ...
              <div class="box-body" id="form">
                ...
                <div class="row">
                  <div class="col-md-6 col-sm-6">
                   ...
                   <div class="col-md-6 col-sm-6">
                      <div class="form-group">
                          <label for="work_station_id">Estación de Trabajo (Destino)</label>
                          <select class="form-control" id="work_station_id" ></select>   
                      </div>
                  </div>
                </div>
              </div>  
...
@section('js')
    ...
    <script>
      ...
      view = 0;
       ...
       $(document).ready(function() {
            ...
            AWApi.get('{{ url('/api/locations') }}',function (response) {
                ...
                AWApi.get('{{ url('/api/work_stations') }}',function (response) {
                  ...
                  for (var i = 0; i < response.data.work_stations.length; i++) {
                    $('<option />', {value: response.data.work_stations[i].id, text: response.data.work_stations[i].name }).appendTo($("#work_station_id"));
                  }

                   AWApi.get('{{ url('/api/work_orders') }}/{{ $id }}', loadForm);
            ...
            (function setFieldValues() {
                ...
                fieldValues['work_station_id'] = 'work_station_id';
            ...
            tbl_orderitems = $('#stock').DataTable( {
                ...
                "ajax": function (data, callback, settings) {
                    ...
                    AWApi.get('{{ url('/api/work_orders' ) }}/{{$id}}/items', function (response) {
            ...
            $("#goBack").click(function(){
              window.location.href = "{{ url( '/work_orders' ) }}";
            });
            ...
            $("#editBtn").click(function(){
              ...
              data.append('work_station_id', $('#work_station_id').val());
              data.append('location_id', $('#location_id').val());
              location_id = $('#location_id').val();
              data.append('date', moment($("#date").val(), 'DD/MM/YYYY').format('YYYY-MM-DD'));
              AWApi.put('{{url('api/work_orders')}}/{{$id}}', data, function (response) {
                submit("", response, "Actualizar Orden de Trabajo");
              });
            });
            ...
            $("#saveDetailsBtn").click(function(){
              for (var i = 0; i < order_items.length; i++) {
                var data = new FormData();
                data.append('quantity', $("#quantity_"+order_items[i].work_order_item_id).val());
                data.append('unit_of_measure_id', $("#uom_id_"+order_items[i].work_order_item_id).val());
                AWApi.put('{{url('api/work_order_items')}}/'+order_items[i].work_order_item_id, data, function (response) {
                  submit("", response, "Actualizar Listado de Productos");
                });
                console.log($("#uom_id_"+order_items[i].work_order_item_id).val());
                console.log($("#quantity_"+order_items[i].work_order_item_id).val());
              }
      ...
      function delOrderItem(id) {
        AWApi.delete('{{url('api/work_order_items')}}/'+id,function (response) {
      ...
      function publishOrder(id) {
            swal({
                title: "Publicar Orden de Trabajo",
            ...
            function () {
                ...
                AWApi.put('{{ url('/api/work_orders') }}/' + id, data, function(datas){
                    submit("", datas, "Publicar Orden de Trabajo");
                    AWApi.get('{{ url('/api/work_orders') }}/{{ $id }}', loadForm);
                });
            });
        ...
        function approveOrder(id) {
            ...
            function () {
                ...
                AWApi.put('{{ url('/api/work_orders') }}/' + id, data, function(datas){
                    submit("", datas, "Aprobar Orden de Trabajo");
                    AWApi.get('{{ url('/api/work_orders') }}/{{ $id }}', loadForm);
                });
            });
        }
      ...
      function loadForm(datas) {
                console.log(datas.data.location_id);
                location_id = datas.data.location_id;
                $('#order_code').text(datas.data.code);
                $('#breadcrumb_active').text(datas.data.code);
                $('#code').val(datas.data.code);
                $('#work_station_id').val(datas.data.work_station_id);
